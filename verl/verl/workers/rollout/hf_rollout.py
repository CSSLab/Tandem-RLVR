...
class HFRollout(BaseRollout):
    def __init__(self, module: nn.Module, config, model_config=None, device_mesh=None):
        # TANDEM MODIFICATION: Pass dummy values to BaseRollout.__init__ for compatibility
        # HFRollout doesn't actually use model_config or device_mesh
        # Set model_config and device_mesh directly without calling parent __init__ to avoid issues
        self.config = config
    ...
    # TANDEM MODIFICATION: Implement abstract methods from BaseRollout
    async def resume(self, tags: list[str]):
        """Resume rollout weights or kv cache in GPU memory (no-op for HF rollout)."""
        pass

    async def update_weights(self, weights, **kwargs):
        """Update the weights of the rollout model (no-op for HF rollout, weights shared with FSDP module)."""
        pass

    async def release(self):
        """Release weights and kv cache in GPU memory (no-op for HF rollout)."""
        pass
    # END TANDEM MODIFICATION
