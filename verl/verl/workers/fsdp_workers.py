...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _vllm_validation_generate(self, prompts: DataProto):
        ...
        # Update hot model weights before each validation
        ...
        for i in range(batch_size):
            prompt_ids = idx[i][attention_mask[i] == 1].tolist()
        ...
        logger.info("Destroying vLLM engine and freeing GPU memory for tandem generation...")
        if hasattr(self, '_vllm_engine'):
            try:
                if hasattr(self._vllm_engine, 'llm_engine'):
                    if hasattr(self._vllm_engine.llm_engine, 'driver_worker'):
                ...
                logger.warning(f"Error cleaning vLLM engine components: {e}")

            del self._vllm_engine

        if hasattr(self, '_vllm_sampling_params'):
            del self._vllm_sampling_params
    ...
    def _tandem_generate_sequences(self, prompts: DataProto, tandem_config):
        ...
        timing_generate = {}
        logger.info("=" * 80)
        logger.info("TANDEM: Starting _tandem_generate_sequences")
        logger.info("=" * 80)
        ...
        if not hasattr(self, '_tandem_rollout_instance'):
            logger.info("Initializing TandemRollout with frozen model...")
            frozen_model = self._load_tandem_frozen_model(tandem_config)

            # TANDEM: Extract non-FSDP model for rollout (FSDP with CPU offload doesn't work in ThreadPoolExecutor)
            hot_model = self._get_non_fsdp_model_for_tandem_rollout(tandem_config)

            # TANDEM: Add response_length to tandem_config from rollout config
            tandem_config_with_response_length = dict(tandem_config)
            tandem_config_with_response_length['response_length'] = self.config.rollout.response_length
            logger.info(f"TandemRollout response_length set to: {tandem_config_with_response_length['response_length']}")

            self._tandem_rollout_instance = TandemRollout(
                model_a=hot_model,
                model_b=frozen_model,
                tokenizer=self.tokenizer,
                config=tandem_config_with_response_length
            )
            self._tandem_rollout_instance.prob_a = tandem_config.get("prob_a", 0.5)
            logger.info(f"TandemRollout initialized with prob_a={self._tandem_rollout_instance.prob_a}")
        else:
            # TANDEM: Update hot model weights from FSDP before generation
            self._update_hot_model_weights_from_fsdp()

        with simple_timer("generate_sequences", timing_generate):
            output = self._tandem_rollout_instance.generate_sequences(prompts)
    ...
    def _update_hot_model_weights_from_fsdp(self):
        ...
        if not hasattr(self, '_hot_model_cache'):
            return

        logger.info("Updating hot model weights from FSDP model...")
        from verl.utils.fsdp_utils import get_fsdp_full_state_dict, load_fsdp_model_to_gpu, offload_fsdp_model_to_cpu
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
        ...
        if not isinstance(fsdp_model, FSDP):
            logger.info("Model is not FSDP, no weight update needed")
            return

        # CRITICAL: Move FSDP parameters to GPU before extracting state dict
        logger.info("Loading FSDP model to GPU temporarily for weight update...")
        load_fsdp_model_to_gpu(fsdp_model)

        # Extract full state dict from FSDP model
        state_dict = get_fsdp_full_state_dict(fsdp_model, offload_to_cpu=False, rank0_only=False)

        # Offload back to CPU
        logger.info("Offloading FSDP model back to CPU...")
        offload_fsdp_model_to_cpu(fsdp_model)

        # Load into the cached hot model
        self._hot_model_cache.load_state_dict(state_dict, strict=True)
        logger.info("Hot model weights updated successfully")

    def _get_non_fsdp_model_for_tandem_rollout(self, tandem_config):
        """Extract a non-FSDP model on cuda:0 for tandem rollout.

        FSDP models with CPU offload don't work in ThreadPoolExecutor because
        the parameters are on CPU and need special context managers to access.
        We need a regular model with all parameters on GPU.
        """
        import torch
        from transformers import AutoModelForCausalLM
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
        ...
        # Check if hot model already cached
        if hasattr(self, '_hot_model_cache'):
            logger.info("Using cached hot model for tandem rollout")
            return self._hot_model_cache

        logger.info("Creating non-FSDP hot model for tandem rollout...")
        ...
        if not isinstance(fsdp_model, FSDP):
            logger.info("Model is not FSDP, using directly")
            self._hot_model_cache = fsdp_model
            return fsdp_model

        # Extract full state dict from FSDP model
        logger.info("Extracting full state dict from FSDP model...")
        from verl.utils.fsdp_utils import get_fsdp_full_state_dict, load_fsdp_model_to_gpu, offload_fsdp_model_to_cpu

        # CRITICAL: Move FSDP parameters to GPU before extracting state dict
        # Otherwise get_fsdp_full_state_dict will fail with "tensor on cpu" error
        logger.info("Loading FSDP model to GPU temporarily for state dict extraction...")
        load_fsdp_model_to_gpu(fsdp_model)

        state_dict = get_fsdp_full_state_dict(fsdp_model, offload_to_cpu=False, rank0_only=False)

        # Offload back to CPU after extraction
        logger.info("Offloading FSDP model back to CPU...")
        offload_fsdp_model_to_cpu(fsdp_model)

        # Load a fresh model on cuda:0
        model_path = self.config.model.path
        logger.info(f"Loading fresh model from {model_path} on {target_device}...")
        hot_model = AutoModelForCausalLM.from_pretrained(
            model_path,
            torch_dtype=torch.bfloat16,
        ...
        # Load the FSDP weights into the fresh model
        logger.info("Loading FSDP weights into fresh model...")
        hot_model.load_state_dict(state_dict, strict=True)

        hot_model.eval()
        logger.info(f"Hot model ready on {target_device}")

        # Cache the hot model
        self._hot_model_cache = hot_model
        return hot_model

    def _load_tandem_frozen_model(self, tandem_config):
        import torch
        from copy import deepcopy

        # Check if frozen model already cached
        if hasattr(self, '_frozen_model_cache'):
            logger.info("Using cached frozen model")
            return self._frozen_model_cache

        frozen_path = tandem_config.get("frozen_model_path", None)
        if frozen_path is None:
            logger.warning("No frozen_model_path specified, using current model as frozen (will be identical to hot model)")
            rollout_model = self.rollout.model if hasattr(self.rollout, 'model') else self.model
            frozen_model = deepcopy(rollout_model)
            frozen_model.eval()
            for param in frozen_model.parameters():
                param.requires_grad = False
            self._frozen_model_cache = frozen_model
            return frozen_model

        logger.info(f"Loading frozen model from {frozen_path}")
        target_device = torch.device("cuda:0")
        logger.info(f"Loading frozen model to cuda:0 (Ray actor only sees one GPU, parallel execution via ThreadPoolExecutor)")

        # TANDEM MODIFICATION: Handle both local paths and HuggingFace model IDs
        from transformers import AutoModelForCausalLM
        import os

        # Check if it's a local directory or a HuggingFace model ID / checkpoint file
        if os.path.isdir(frozen_path):
            # Local directory with model files
            logger.info(f"Loading frozen model from local directory: {frozen_path}")
            ...
            logger.info("Frozen model loaded from local directory")
        elif os.path.isfile(frozen_path):
            # Local checkpoint file (state dict)
            logger.info(f"Loading frozen model from checkpoint file: {frozen_path}")
            # Can't deepcopy FSDP model, so load from pretrained first
            model_path = self.config.model.path
            frozen_model = AutoModelForCausalLM.from_pretrained(
                model_path,
                torch_dtype=torch.bfloat16,
            ...
            state_dict = torch.load(frozen_path, map_location=target_device)
            ...
            logger.info(f"Loading frozen model from HuggingFace Hub: {frozen_path} (this may use cached version)")
            logger.info(f"HF_HOME is set to: {os.environ.get('HF_HOME', 'not set')}")
            frozen_model = AutoModelForCausalLM.from_pretrained(
                frozen_path,
                torch_dtype=torch.bfloat16,
                device_map={"": target_device},
                trust_remote_code=True,
                cache_dir=os.environ.get('HF_HOME', None)  # Use HF_HOME for caching
            )
            logger.info("Frozen model loaded from HuggingFace Hub")
        # END TANDEM MODIFICATION
        ...
        logger.info(f"Frozen model loaded successfully on {target_device}")
        # Cache the frozen model to avoid reloading
        self._frozen_model_cache = frozen_model
        return frozen_model
    # END TANDEM MODIFICATION
